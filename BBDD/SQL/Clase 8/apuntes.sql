...
select last_name as "Last Name", salary as "Salary", (salary * 0.1) + salary as "Salary incremented a 10%"
from employees;

--Agregando una columna llamada comission_pct
alter table employees add column comission_pct float;

select last_name, salary, comission_pct, comission_pct/100 * salary as pct, salary + comission_pct/100 * salary as salary2
from employees;

-- NULL -> NADA
-- Comilla sencilla para literales o cadenas, comillas dobles para alias de las columnas
select first_name || ' ' || last_name as Full_Name, salary 
from employees;

-- Evitar elementos repetidos (Muestra los que son diferentes)
select distinct department_id
from employees;

-- Selecciona la combinación de todos los elementos diferentes
...
-- Clase 9 FUnciones de grupo
select max(salary), min(salary), avg(salary), sum(salary), count(*), count (distinct department_id), count(comission_pct)
from employees;

-- Busca todas las combinaciones posibles entre id_dep y job_id y saca el salario promedio 
...
update employees set last_name = substr(upper(last_name),1,1) || substr(last_name,2),
first_name = substr(upper(first_name),1,1) || substr(first_name ,2); 
